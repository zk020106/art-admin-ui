      ...
      <div class="flex h-[calc(100%-70px)] flex-col">
        ...
        >
          <template v-for="(message, index) in messages" :key="index">
            <div
              class="mb-7.5 flex w-full items-start gap-2" :class="[
                message.isMe ? 'flex-row-reverse' : 'flex-row',
              ]"
            >
              <ElAvatar :size="32" :src="message.avatar" class="shrink-0" />
              <div
                class="flex max-w-[70%] flex-col" :class="[message.isMe ? 'items-end' : 'items-start']"
              >
                <div
                  class="mb-1 flex gap-2 text-xs" :class="[
                    message.isMe ? 'flex-row-reverse' : 'flex-row',
                  ]"
                >
                ...
                <div
                  class="rounded-md px-3.5 py-2.5 text-sm leading-[1.4] text-g-900" :class="[
        ...
        <div class="px-4 pt-4">
          ...
          >
            <template #append>
              <div class="flex gap-2 py-2">
                ...
                <ElButton v-ripple type="primary" @click="sendMessage">
                  发送
                </ElButton>
              </div>
            </template>
          ...
          <div class="mt-3 flex-cb">
            ...
            <ElButton v-ripple type="primary" class="min-w-20" @click="sendMessage">
              发送
            </ElButton>
          </div>
        </div>
