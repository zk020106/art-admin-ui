...
<template>
  <div class="page-content">
    <h3 class="mb-5 text-xl font-normal">
      标签页操作
    </h3>
    ...
    <ElCard class="mb-7.5" header="修改标签标题" shadow="never">
      <div class="flex gap-2">
        ...
        <ElButton type="primary" :disabled="!newTabTitle.trim()" @click="handleUpdateTabTitle">
          修改
        </ElButton>
        <ElButton @click="handleResetTabTitle">
          重置
        </ElButton>
      </div>
    </ElCard>
    ...
    <ElCard class="mb-7.5" header="获取标签页信息" shadow="never">
      <div class="mb-4">
        <p class="m-0 mb-2 text-sm text-g-600">
          当前标签页信息：{{ currentTab }}
        </p>
      </div>
      <ElRow :gutter="20">
    ...
    <ElCard class="mb-7.5" header="关闭标签页" shadow="never">
      <ElRow :gutter="20">
        <ElCol :span="24">
          ...
          <ElButton type="danger" plain @click="handleCloseAllTab">
            关闭所有标签
          </ElButton>
        </ElCol>
      </ElRow>
